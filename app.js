...
function renderGeoAdminList(){
 ...
 body.innerHTML=rows.map(s=>`<tr><td><b>${escapeHtml(s.name||s.id)}</b><small>ID ${escapeHtml(String(s.id))}</small></td><td>${escapeHtml(s.address||"—")}</td><td class="geo-lat">${s.latitude??"—"}</td><td class="geo-lng">${s.longitude??"—"}</td><td><button class="geo-row-btn" data-geo-id="${escapeHtml(String(s.id))}">${s.latitude!=null&&s.longitude!=null?"Recalculeaza":"Gaseste coordonatele"}</button></td></tr>`).join("");
...
async function geocodeOneStore(id,b){
 ...
 if(!s?.address){st.textContent="Nu exista adresa pentru acest magazin.";return;}
 const old=b.textContent;b.disabled=true;b.textContent="Se cauta...";
 try{
  const queries=[`${s.address}, Romania`,s.address];
  let a=[];
  for(const text of queries){
   const q=encodeURIComponent(text);
   const r=await fetch(`https://nominatim.openstreetmap.org/search?format=jsonv2&limit=1&countrycodes=ro&addressdetails=1&q=${q}`,{headers:{"Accept":"application/json","Accept-Language":"ro"}});
   if(!r.ok) throw new Error(`HTTP ${r.status}`);
   a=await r.json();
   if(Array.isArray(a)&&a.length)break;
  }
  if(!Array.isArray(a)||!a.length){st.textContent=`Nu am gasit coordonatele pentru ${s.name}.`;return;}
  ...
  st.textContent=`${s.name}: ${latitude.toFixed(7)}, ${longitude.toFixed(7)} salvate.`;
  renderGeoAdminList();
 }catch(e){console.error("Geocodare:",e);st.textContent=`Eroare la cautare: ${e.message||e}`;}
